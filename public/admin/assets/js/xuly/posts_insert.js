...
function ValidateData() {
	...
	if (check_isset_loai_size($("#slug").val()) ==="true") {
		sweetAlter("error" , "Error" , "Tiêu đề bài viết đã tồn tại!!!");
	    return false;
	}
	if ($(".file-upload-info").val() ==="") {
	    sweetAlter("error" , "Error" , "Ảnh không được để trống !!!");
	    return false;
	}
	
}

function check_isset_loai_size(val) {
	// body...
	var str = "";
	$.ajax({
		url: './admin/posts/check_isset_ten_bai_viet/'+val,
		type: 'GET',
		async: false,
